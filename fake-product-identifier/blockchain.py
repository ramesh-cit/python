...
class Blockchain:
    def __init__(self):
        self.chain = list()
        available = self.load_block_file()
        if not available:
    ...
    def is_item_in_blockchain(self, data: str) -> bool:       
        block_index = 1
        while block_index < len(self.chain):
            block = self.chain[block_index]
            # Check if the data is in this block
            if (block["data"] == data):
                return True
    ...
    def load_block_file(self):    
        with open('block-chain.dat', 'r', encoding='UTF-8') as file:
    ...
    def write_block_file(self):
        textfile = open("block-chain.dat", "w+")        
        for element in self.chain:
            jsonText = _json.dumps(element)
            textfile.write(jsonText +"\n")
        textfile.close()        
